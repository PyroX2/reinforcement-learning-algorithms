...
NUM_STEPS = 500_000
LEARNING_RATE = 3e-4
NUM_EPISODES_TO_AVERAGE = 1
UPDATE_TARGET_NETWORK_STEPS = 10
...
class DQN(nn.Module):
    def __init__(self, obs_space, action_space):
        ...
        self.fc = nn.Sequential(nn.Linear(obs_space, 512),
                                nn.ReLU(),
                                nn.Linear(512, 128),
                                nn.ReLU(),
...
class ReplayBuffer:
    ...
    def get_batch(self, batch_size):
        ...
        batch_indices = torch.randperm(len(self._memory["state"])-NUM_FRAMES_STACKED)[:batch_size] + NUM_FRAMES_STACKED
        ...
        indices = batch_indices.unsqueeze(1) - offsets  # Shape: (batch_size, 4)

        def get_stacked_frames(array):
            return array[indices.flatten()].view(len(batch_indices), NUM_FRAMES_STACKED, -1)
            
        states = get_stacked_frames(torch.tensor(self._memory["state"], dtype=torch.float32).to(device))
        actions = torch.tensor(self._memory["action"], dtype=torch.int64).to(device)[batch_indices]
        new_states = get_stacked_frames(torch.tensor(self._memory["new_state"], dtype=torch.float32).to(device))
        rewards = torch.tensor(self._memory["reward"], dtype=torch.float32).to(device)[batch_indices]

        states = states.flatten(1, -1)
        new_states = new_states.flatten(1, -1)
...
policy_network = DQN(obs_space * NUM_FRAMES_STACKED, action_space)
# policy_network.load_state_dict(torch.load("dqn_breakout_trained.pth", weights_only=True))

target_network = DQN(obs_space * NUM_FRAMES_STACKED, action_space)
...
stacked_frames_state = []
stacked_frames_state.append(state.tolist())

episode_reward = torch.tensor(0, dtype=torch.float64)
for step in range(NUM_STEPS):
    if len(stacked_frames_state) < NUM_FRAMES_STACKED:
        new_state, reward, terminated, truncated, _ = env.step(0)
        stacked_frames_state.append(new_state.tolist())
        continue
    ...
    #     action = 1 # Fire
    # else:
    stacked_frames_state_tensor = torch.tensor(stacked_frames_state, dtype=torch.float32).flatten()
    action = get_action(stacked_frames_state_tensor.to(device), policy_network, action_space, eps_threshold)

    new_state, reward, terminated, truncated, _ = env.step(action)

    stacked_frames_state.append(new_state.tolist())
    stacked_frames_state.pop(0)
    ...
    replay_buffer.update(state.tolist(), action, new_state.tolist(), reward)
